...
class LLM:
    def __init__(self, api_key: str='', file_path: str = '../event_list.json', model_type: str = "ipex_llm",model_path:str='qwen2chat_int4'):
        self.file_path = file_path
        self.model_type = model_type
        self.res = ''
    ...
    def get_event_list(self, text: str):
        if self.model_type == 'deepseek':
            ...
            prompt = f"您是一名地理分析师，请根据以下内容划分事件，并按照事件前后顺序排列：{text}"
            return self.ipex_llm_generate(prompt)
    ...
    def process_event(self, event_text: str, language: str = '英语'):
        if self.model_type == 'deepseek':
            ...
            prompt = f"请根据以下内容分析并生成事件信息：{event_text}"
            return self.ipex_llm_generate(prompt)
